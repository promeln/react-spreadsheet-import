...
const ValidationStep = ({ initialData, file }) => {
    const { translations, fields, onClose, onSubmit, rowHook, tableHook, submitFailHook } = useRsi.useRsi();
    ...
    const submitData = () => {
        ...
        if (res?.then) { //if onSubmit returns Promise
            res.then((res) => {
                if (res?.error) {
                    setData(dataMutations.addErrorsAndRunHooks(data, [], undefined, undefined, submitFailHook, res.error));
                    return;
                }
